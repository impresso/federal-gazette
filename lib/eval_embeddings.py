...
#  Alex Flückiger <alex.flueckiger@gmail.com>
#
...
parser = argparse.ArgumentParser(
    description="Script for creating testsets and evaluating word vector models"
)
...
parser.add_argument(
    ...
    "--create",
    action="store_true",
    help="if set, create testsets before evaluating",
)
parser.add_argument(
    "-u",
    "--umlauts",
    action="store_true",
    help="if set, create additional testsets with transformed umlauts and use them instead",
)
parser.add_argument(
    "-t",
    "--topn",
    type=int,
    default=10,
    help="check the top n result (correct answer under top n answeres)",
)
...
parser.add_argument(
    ...
    default=False,
    dest="lower",
    help="evaluate word vectors in lowercase regardless of its grammatical form",
)

args = parser.parse_args()
TARGET_SYN = "lib/embed_eval_data/syntactic.questions"
TARGET_SEM_OP = "lib/embed_eval_data/semantic_op.questions"
TARGET_SEM_BM = "lib/embed_eval_data/semantic_bm.questions"
TARGET_SEM_DF = "lib/embed_eval_data/semantic_df.questions"
SRC_NOUNS = "lib/embed_eval_data/nouns.txt"
SRC_ADJECTIVES = "lib/embed_eval_data/adjectives.txt"
SRC_VERBS = "lib/embed_eval_data/verbs.txt"
SRC_BESTMATCH = "lib/embed_eval_data/bestmatch.txt"
SRC_DOESNTFIT = "lib/embed_eval_data/doesntfit.txt"
SRC_OPPOSITE = "lib/embed_eval_data/opposite.txt"
PATTERN_SYN = [
...
logging.basicConfig(
    filename=args.model.strip() + ".result",
...
def test_most_similar(model, src, label="most similar", topn=10, lower=False):
    """
    ...
    for question in questions:
        ...
        if all(x in model.index2word for x in words):
            num_questions += 1
            best_matches = model.most_similar(
                positive=[words[1], words[2]], negative=[words[0]], topn=topn
            )
            # best match
    ...
    coverage = (
        round(num_questions / float(num_lines) * 100, 1) if num_lines > 0 else 0.0
    )
    # log result
    logging.info(
        label
        + " correct:  {0}% ({1}/{2})".format(correct_matches, num_right, num_questions)
    )
    logging.info(
        label
        + " top {0}:   {1}% ({2}/{3})".format(
            topn, topn_matches, num_topn, num_questions
        )
    )
    logging.info(
        label + " coverage: {0}% ({1}/{2})".format(coverage, num_questions, num_lines)
    )


def test_most_similar_groups(model, src, topn=10, lower=False):
    """
    Tests given model to most similar word.
    ...
    # log result
    logging.info(
        "total correct:  {0}% ({1}/{2})".format(
            correct_matches, num_right, num_questions
        )
    )
    logging.info(
        "total top {0}:   {1}% ({2}/{3})".format(
            topn, topn_matches, num_topn, num_questions
        )
    )
    logging.info(
        "total coverage: {0}% ({1}/{2})".format(coverage, num_questions, num_lines)
    )


def test_doesnt_fit(model, src, lower=False):
    """
    ...
    # log result
    logging.info(
        "doesn't fit correct:  {0}% ({1}/{2})".format(
            correct_matches, num_right, num_questions
        )
    )
    logging.info(
        "doesn't fit coverage: {0}% ({1}/{2})".format(
            coverage, num_questions, num_lines
        )
    )
...
trained_model = gensim.models.KeyedVectors.load_word2vec_format(
    args.model.strip(), binary=is_binary
)
...
test_most_similar_groups(
    trained_model,
    TARGET_SYN + ".nouml" if args.umlauts else TARGET_SYN,
    args.topn,
    args.lower,
)
...
test_most_similar(
    trained_model,
    TARGET_SEM_OP + ".nouml" if args.umlauts else TARGET_SEM_OP,
    "opposite",
    args.topn,
    args.lower,
)
test_most_similar(
    trained_model,
    TARGET_SEM_BM + ".nouml" if args.umlauts else TARGET_SEM_BM,
    "best match",
    args.topn,
    args.lower,
)
test_doesnt_fit(
    trained_model,
    TARGET_SEM_DF + ".nouml" if args.umlauts else TARGET_SEM_DF,
    args.lower,
)
